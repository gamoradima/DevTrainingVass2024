namespace Terrasoft.Core.Process
{
	...
	public class UsrCalculateAveragePriceProcessMethodsWrapper : ProcessModel
	{
		...
		private bool ScriptTask1Execute(ProcessExecutingContext context) {
			...
			var priceColumn = esq.AddColumn("UsrPriceGBP");  // SELECT UsrPriceGBP as UsrPriceGBP, UsrArea as UsrArea FROM UsrRealty WHERE ...
			...
			foreach(var entity in entityCollection) {
				...
				totalGBP = totalGBP + price;
				totalArea = totalArea + area;
			}
			
			decimal result = 0;
			if (totalArea > 0) {
				result = totalGBP / totalArea;
			}
			
			Set("AvgPriceGBPParameter", result);
